...
const AssigneeSelect = ({ value, options = [], onChange, placeholder = 'Seleccionar encargado', className = '' }) => {
    ...
    <div className={`relative ${className}`}>
      ...
      >
        <span className="flex items-center gap-2">
          <span className="inline-block w-6 h-6 rounded-full bg-gray-200 text-gray-600 flex items-center justify-center text-xs font-medium">
            {selected ? selected.charAt(0).toUpperCase() : '—'}
          </span>
      ...
      {open && (
        <div ref={panelRef} className="absolute z-10 mt-2 w-full rounded-lg border border-gray-200 bg-white shadow-lg overflow-hidden">
          <ul className="py-1 max-h-60 overflow-auto">
            {options.length === 0 && (
              <li className="px-3 py-2 text-sm text-gray-500">No hay miembros</li>
            )}
            {options.map((opt) => (
              <li key={opt}>
                <button
